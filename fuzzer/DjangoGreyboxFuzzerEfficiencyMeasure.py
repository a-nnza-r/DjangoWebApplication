...
class DjMutator(AbstractMutator):
    ...
    def __init__(self, config_path: str = "fuzzer/fuzzer_config.yaml", use_constraints: bool = True) -> None: # Adjusted default path
        """
        ...
        if self.use_constraints:
            try:
                if config_path and os.path.exists(config_path): # Check if path is valid
...
class DjGreyboxFuzzer(AbstractGreyboxFuzzer):
    ...
    def __init__(
        ...
        config_path: str = "fuzzer/fuzzer_config.yaml", # Adjusted default path
    ...
    def start_server(self):
        ...
        try:
            ...
            self.server_process = subprocess.Popen(
                cmd,
                stdout=self.stdout_log_file, # Redirect stdout to file
                stderr=self.stderr_log_file, # Redirect stderr to file
                text=True,
            ...
            if poll_result is None:
                ...
                # self._log_server_output() # Output is now in files
    ...
    def stop_server(self):
        ...
        try:
            ...
            # self._log_server_output() # Output is in files
        except subprocess.TimeoutExpired:
    ...
    def run(self) -> None:
        ...
        try: # Wrap in try/finally to ensure server stops
            ...
            # --- MODIFICATION: Add timing columns to header ---
                ...
                for mutation_index_in_iteration in range(mutations_to_attempt):
                    ...
                    if self.server_process and self.server_process.poll() is None:
                        ...
                        try:
                            ...
                            # --- MODIFICATION: Capture execution time even on exception ---
                            execution_end_time = time.perf_counter()
                    ...
                    log_entry = {
                        ...
                        "total_unique_arcs": len(self.global_unique_arcs),
                        "total_unique_paths": len(self.global_unique_path_hashes),
                        # --- MODIFICATION: Add timings to log entry ---
...
def main() -> None:
    ...
    parser.add_argument('--config', type=str, default="fuzzer/fuzzer_config.yaml", # Adjusted default path
    ...
    mutator = DjMutator(config_path=config_path if config_path else "fuzzer/fuzzer_config.yaml") # Pass original path or default
    is_interesting = DjIsInteresting()
