...
from fuzzer.abstractUpdated import (
...
interesting_logger = logging.getLogger(
    "InterestingMutationsLogger"
)  # For interesting mutations
# --- End logger setup ---
...
class DjIsInteresting(AbstractIsInteresting):
    ...
    def _count_to_bucket_index(count: int) -> int:
        ...
        if count <= 0:
            return -1  # Should not happen for actual hits
        if count == 1:
            return 0
        if count == 2:
            return 1
    ...
    def __call__(
        ...
        for arc, current_hit_count in arc_hit_counts.items():
            ...
            current_bucket_index = DjIsInteresting._count_to_bucket_index(
                current_hit_count
            )
...
class DjPowerSchedule(AbstractPowerSchedule):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:  # Accept config
        ...
        logging.info(
            f"PowerSchedule initialized with energy_const={self.energy_const}, max_energy={self.max_energy}"
        )
    ...
    def assignEnergy(self, input_seed: Dict[str, Any], average_f: float) -> int:
        ...
        if (
            average_f > 0 and f_val > average_f * 2
        ):  # Example: Penalize if f is > 2x average
            logging.debug(
                f"Assigning 0 energy to seed (f={f_val} > 2*avg_f={average_f*2:.2f})"
            )
            return 0
        ...
        try:
            ...
            logging.warning(
                f"Overflow calculating exponent for s_val={s_val}. Using max energy."
            )
...
class DjMutator(AbstractMutator):
    ...
    def __init__(
        self,
        config_path: str = "DjangoWebApplication/fuzzer/fuzzer_config.yaml",
                ...
                if os.path.exists(config_path):
                    with open(config_path, "r") as f:
                        full_config = yaml.safe_load(
                            f
                        )  # Load the full config temporarily
                    if full_config and "input_structure" in full_config:
                        ...
                        logging.info(
                            f"Mutator loaded input structure from: {config_path}"
                        )
                    else:
                        logging.warning(
                            f"Config file found at {config_path}, but missing 'input_structure' key. Constraint mutation may be limited."
                        )
                else:
                    logging.warning(
                        f"No fuzzer configuration file found at {config_path}. Constraint mutation may be limited."
                    )
            except yaml.YAMLError as e:
                logging.error(
                    f"Error parsing YAML configuration file {config_path} for mutator: {e}"
                )
                self.input_structure = {}
            except Exception as e:
                logging.error(
                    f"Error loading configuration file {config_path} for mutator: {e}"
                )
                self.input_structure = {}
        else:
        ...
        if isinstance(original_value, str):
            ...
            if self.use_constraints:
                ...
                str_mutation_methods: List[
                    ...
                    lambda s, constraints=None: random.choice(
                        self.common_words
                    ),  # Replace with common word (ignore constraints)
                    lambda s, constraints=None: random.choice(
                        self.sql_patterns
                    ),  # Replace with SQL pattern (ignore constraints)
                    lambda s, constraints=None: random.choice(
                        self.xss_patterns
                    ),  # Replace with XSS pattern (ignore constraints)
                    lambda s, constraints=None: "A"
                    * random.randint(1, 100),  # Replace with random length 'A's
                ]
            ...
            try:
                ...
                logging.warning(
                    f"Error during string mutation for field '{field}' (value: {original_value}, constraints: {constraints}, use_constraints: {self.use_constraints}): {e}. Falling back."
                )
                mutated_data[field] = random.choice(
                    [original_value, "", "mutated_error"]
                )  # Fallback
            ...
            if self.use_constraints:
                ...
                int_mutation_methods: List[
                    Callable[[int, Optional[Dict[str, Any]]], int]
                ] = [
                ...
                int_mutation_methods: List[
                    ...
                    lambda i, constraints=None: i
                    + random.choice([-1, 1, -10, 10, -100, 100]),  # Basic arithmetic
                    lambda i, constraints=None: random.choice(
                        [0, -1, 1, 2**15 - 1, -(2**15)]
                    ),  # Basic extremes
            ...
            try:
                ...
                logging.warning(
                    f"Error during integer mutation for field '{field}' (value: {original_value}, constraints: {constraints}, use_constraints: {self.use_constraints}): {e}. Falling back."
                )
            ...
            if self.use_constraints:
                ...
                _float_only_mutations: List[
                    Callable[[float, Optional[Dict[str, Any]]], float]
                ] = [
                ...
                _float_only_mutations: List[
                    ...
                    lambda f, constraints=None: f
                    + random.uniform(-100.0, 100.0),  # Basic arithmetic
                    lambda f, constraints=None: f
                    * random.uniform(-10.0, 10.0),  # Basic scaling
                    lambda f, constraints=None: random.choice(
                        [0.0, 1.0, -1.0, 1e-5, -1e-5, 1e5, -1e5]
                    ),  # Basic extremes
                ]
            ...
            try:
                if mutation in _float_only_mutations:
                    ...
                    int_result = mutation(
                        int(original_value) if original_value.is_integer() else 0,
                ...
                logging.warning(
                    f"Error during float mutation for field '{field}' (value: {original_value}, constraints: {constraints}, use_constraints: {self.use_constraints}): {e}. Falling back."
                )
    ...
    def byte_insert(
        self, data: str, constraints: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def replace_with_extreme_string(
        self, data: str, constraints: Optional[Dict[str, Any]] = None
    ) -> str:
        """Replace string with extreme values, trying to respect length constraints."""
        min_len = constraints.get("min_length", 0) if constraints else 0
        max_len = (
            constraints.get("max_length") if constraints else float("inf")
        )  # Use infinity if no max
        ...
        extreme_values_generators = [
            ...
            lambda: "\x00"
            * random.randint(
                min_len, min(50, int(max_len)) if max_len != float("inf") else 50
            ),
            lambda: "\xff"
            * random.randint(
                min_len, min(50, int(max_len)) if max_len != float("inf") else 50
            ...
            lambda: "A"
            * random.randint(
                min_len, min(2000, int(max_len)) if max_len != float("inf") else 2000
            ),
            lambda: "Z"
            * random.randint(
                min_len, min(4000, int(max_len)) if max_len != float("inf") else 4000
            ),
            lambda: " "
            * random.randint(
                min_len, min(500, int(max_len)) if max_len != float("inf") else 500
            ),
            lambda: "\n"
            * random.randint(
                min_len, min(50, int(max_len)) if max_len != float("inf") else 50
            ...
            * random.randint(
                min(min_len // 3, 1),
                min(20, int(max_len // 3)) if max_len != float("inf") else 20,
            ),
            lambda: "%s%n"
            * random.randint(
                min(min_len // 3, 1),
        ...
        while attempts < 10:  # Try a few times to generate a valid one
            ...
            try:
                ...
                ValueError
            ):  # Handle potential issues with random range if min_len > max_len etc.
                pass
            attempts += 1

        # Fallback if no valid extreme value generated easily
        fallback_len = random.randint(
            min_len, int(max_len) if max_len != float("inf") else min_len + 10
        )
    ...
    def word_mutation(
        ...
        if len(res) < min_len:
            ...
            if min_len <= len(data) <= max_len:
                ...
                return "Y" * min(
                    min_len, int(max_len) if max_len != float("inf") else min_len
                )

        return res

    def special_char_mutation(
        self, data: str, constraints: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def long_string_mutation(
        self, data: str, constraints: Optional[Dict[str, Any]] = None
    ) -> str:
        """Create long strings, respecting length constraints."""
        min_len = (
            constraints.get("min_length", 1000) if constraints else 1000
        )  # Default min if not specified
        max_len = (
            constraints.get("max_length", 5000) if constraints else 5000
    ...
    def char_swap(self, data: str, constraints: Optional[Dict[str, Any]] = None) -> str:
        ...
        if len(data) < 2:
            return data  # Need at least two chars to swap
        ...
        attempts = 0
        while (
            pos1 == pos2 and attempts < 5
        ):  # Avoid infinite loop on len 1 (though checked above)
            pos2 = random.randint(0, len(s) - 1)
            attempts += 1
        if pos1 == pos2:
            return data  # Give up if couldn't find different positions quickly

        s[pos1], s[pos2] = s[pos2], s[pos1]
        return "".join(s)

    # --- Type-Specific Extreme Value Mutations (Constraint-Aware) ---
    def replace_with_extreme_float(
        self, data: float, constraints: Optional[Dict[str, Any]] = None
    ) -> float:
        ...
        if constraints:
            ...
            if "specific_values" in constraints and isinstance(
                constraints["specific_values"], list
            ):
                potential_values.extend(
                    [
                        float(v)
                        for v in constraints["specific_values"]
        ...
        min_bound = (
            float(constraints.get("min_value", -float("inf")))
            if constraints
            else -float("inf")
        )
        max_bound = (
            float(constraints.get("max_value", float("inf")))
            if constraints
            else float("inf")
        )
        ...
        if not valid_values:
            ...
            if (
                data == data and min_bound <= data <= max_bound
            ):  # Check data is not NaN and within bounds
    ...
    def random_float_mutation(
        self, data: float, constraints: Optional[Dict[str, Any]] = None
    ) -> float:
        """Apply random arithmetic mutations to a float, respecting constraints."""
        # Determine bounds from constraints or use defaults
        min_bound = (
            float(constraints.get("min_value", -sys.float_info.max))
            if constraints
            else -sys.float_info.max
        )
        max_bound = (
            float(constraints.get("max_value", sys.float_info.max))
            if constraints
            else sys.float_info.max
        )
        ...
        try:
            ...
            logging.debug(
                f"Exception during random_float_mutation for {data}. Falling back."
            )
            ...
            return (
                max(min_bound, min(max_bound, fallback_val))
                if min_bound <= max_bound
                else data
            )

    def arith_inc_dec_str(
        self, data: str, constraints: Optional[Dict[str, Any]] = None
    ) -> str:
    ...
    def arith_inc_dec_int(
        self, data: int, constraints: Optional[Dict[str, Any]] = None
    ) -> int:
        ...
        # Determine bounds from constraints or use defaults
        min_bound = (
            constraints.get("min_value", -9223372036854775808)
            if constraints
            else -9223372036854775808
        )
        max_bound = (
            constraints.get("max_value", 9223372036854775807)
            if constraints
    ...
    def arith_inc_dec_float(
        ...
        try:
            ...
            # Fallback if mutation causes issues
            logging.debug(
                f"Exception during arith_inc_dec_float for {data}. Falling back."
            )
    ...
    def replace_with_extreme_int(
        ...
        default_values = [
            ...
            -(2**15),
            2**16 - 1,
            -(2**16 - 1),  # Standard 16-bit limits
            2**31 - 1,
            -(2**31),
            2**32 - 1,
            -(2**32 - 1),  # Standard 32-bit limits
            2**63 - 1,
            -(2**63),  # Standard 64-bit limits
        ]
        ...
        if constraints:
            ...
            if "specific_values" in constraints and isinstance(
                ...
                potential_values.extend(
                    [v for v in constraints["specific_values"] if isinstance(v, int)]
                )

        # Determine bounds for filtering
        min_bound = (
            constraints.get("min_value", -float("inf"))
            if constraints
            else -float("inf")
        )
        max_bound = (
            constraints.get("max_value", float("inf")) if constraints else float("inf")
        )
        ...
        if not valid_values:
            ...
            if min_bound <= data <= max_bound:
                ...
                valid_values.append(
                    int(min_bound)
                )  # Cast needed if min_bound came from float constraint
            else:  # Should only happen if bounds are impossible (-inf, -inf)
                valid_values.append(0)
    ...
    def int_bit_flip(
        ...
        if data == 0:  # Flipping a bit in 0 results in a power of 2
            return 1 << random.randint(
                0, 16
            )  # Flip a bit in the lower 16 bits for practicality
        ...
        try:
            ...
            if not byte_list:
                return data  # Should not happen if data != 0
            ...
            mutated_data = int.from_bytes(
                bytes(byte_list), byteorder="little", signed=(data < 0)
            )
            ...
            logging.warning(
                f"OverflowError during int_bit_flip for {data}. Returning original."
            )
            return data
        except Exception as e:
            logging.warning(
                f"Error during int_bit_flip for {data}: {e}. Returning original."
            )
            return data
    ...
    def int_byte_swap(
        ...
        if data == 0:
            return 0  # Swapping bytes in 0 does nothing
        ...
        try:
            ...
            while idx1 == idx2 and attempts < 5:  # Try to get different indices
                ...
                attempts += 1
            if idx1 == idx2:
                return data  # Give up if couldn't find distinct indices
            ...
            # Convert back to integer
            mutated_data = int.from_bytes(
                bytes(byte_list), byteorder="little", signed=is_signed
            )
            ...
            logging.warning(
                f"OverflowError during int_byte_swap for {data}. Returning original."
            )
            return data
        except Exception as e:
            logging.warning(
                f"Error during int_byte_swap for {data}: {e}. Returning original."
            )
            return data
...
class DjGreyboxFuzzer(AbstractGreyboxFuzzer):
    ...
    def __init__(
        ...
        self.use_constraint_mutation = self.config.get(
            "use_constraint_mutation", True
        )  # Mutator needs to handle this internally
        ...
        self.mutator = DjMutator(
            config_path=self.config_path, use_constraints=self.use_constraint_mutation
        )
        ...
        self.output_base_dir = self.config.get(
            "output_base_dir",
            os.path.join(
                os.path.dirname(__file__), "evaluation_runs", self.experiment_name
            ),
        )
        self.run_duration_hours = self.config.get(
            "run_duration_hours", 0
        )  # Default 0 hours (run indefinitely or until other limits)
        self.run_duration_seconds = (
            self.run_duration_hours * 3600
            if self.run_duration_hours > 0
            else float("inf")
        )

        # --- Configure Coverage Object ---
        # Use the specific coverage data file base name from config
        coverage_data_file_base = self.config.get(
            "coverage_data_file", f".coverage_{self.experiment_name}"
        )
        # Place coverage file inside the run-specific output directory
        self.coverage_data_file_path = os.path.join(
            self.get_run_output_dir(), f"{coverage_data_file_base}"
        )  # No suffix needed if auto_data=False
        self.cov = Coverage(
            branch=True,
        ...
        logging.info(
            f"Coverage configured to use data file: {self.coverage_data_file_path}"
        )
        # --- End Coverage Config ---
        ...
        self.total_mutation_limit = self.config.get(
            "total_mutation_limit", 1000000
        )  # Use higher default from new configs
        ...
        self.last_successful_input_path_hash: str = (
            "INITIAL"  # Path hash of last successful input
        )
    ...
    def _log_unique_error(self, bug_report: Dict[str, Any], path_hash: str) -> None:
        ...
        if is_placeholder:
            ...
            logging.info(
                f"Added unique error instance (placeholder path '{path_hash}'): {self.safe_repr(bug_report)}"
            )
        else:
            ...
            if error_signature not in self.unique_errors:
                ...
                self.unique_error_details.append(bug_report)
                logging.info(
                    f"Added UNIQUE error signature ({error_signature}): {self.safe_repr(bug_report)}"
                )
            # else: Signature already seen, do not log again as unique

    def _generate_error_signature(
        self, bug_report: Dict[str, Any], path_hash: str
    ) -> Tuple[str, str, str]:
        ...
        if error_type == "http_error":
            ...
            "timeout"
        ):  # Covers timeout_alive, timeout_crash, timeout_memory_dos
            ...
            error_msg = str(bug_report.get("error", ""))[
                :50
            ]  # Use first 50 chars of error
        ...
        try:
            if os.path.exists(config_path):
                with open(config_path, "r") as f:
                    config = (
                        yaml.safe_load(f) or {}
                    )  # Ensure it's a dict even if file is empty
                logging.info(f"Fuzzer loaded configuration from: {config_path}")
            else:
                logging.warning(
                    f"Fuzzer configuration file not found at {config_path}. Using defaults."
                )
        except yaml.YAMLError as e:
            logging.error(
                f"Error parsing YAML configuration file {config_path}: {e}. Using defaults."
            )
        except Exception as e:
            logging.error(
                f"Error loading configuration file {config_path}: {e}. Using defaults."
            )
        return config
    ...
    def start_server(self):
            ...
            for filename in os.listdir("."):
                if filename.startswith(coverage_data_file_base + "."):
                    try:
                        ...
                        logging.warning(
                            f"Error removing old coverage file {filename}: {e}"
                        )
        except Exception as e:
        ...
        if not target_script:
            logging.critical(
                "`target_script` not defined in configuration. Cannot start server."
            )
            return
        if not os.path.exists(target_script):
            logging.critical(
                f"Target script '{target_script}' not found. Cannot start server."
            )
            return
        if not isinstance(target_args, list):
            logging.warning(
                f"`target_args` in config is not a list. Using empty list. Value: {target_args}"
            )
            target_args = []
        ...
        cmd = [
            ...
            "--branch",
            "--data-file",
            coverage_data_file,  # Use configured data file
        ]
        ...
        try:
            ...
            if poll_result is None:
                ...
                logging.error(
                    f"Server failed to start or exited immediately with code {poll_result}."
                )
    ...
    def stop_server(self):
        ...
        try:
            ...
            logging.warning(
                f"Server process (PID: {pid}) did not terminate after SIGINT. Sending SIGTERM..."
            )
            try:
                ...
                logging.error(
                    f"Server process (PID: {pid}) did not terminate after SIGTERM. Sending SIGKILL."
                )
                ...
                logging.error(
                    f"Error sending SIGTERM/SIGKILL to server (PID: {pid}): {e}",
                    exc_info=True,
                )
        except Exception as e:
            logging.error(
                f"Error stopping server process (PID: {pid}): {e}", exc_info=True
            )
            # Ensure process is killed if any error occurs during shutdown sequence
            try:
                if self.server_process.poll() is None:
                    logging.warning(
                        f"Force killing server process (PID: {pid}) due to error during shutdown."
                    )
                    os.killpg(os.getpgid(pid), signal.SIGKILL)
    ...
    def send_request(self, input_data: Dict[str, Any]) -> Optional[requests.Response]:
        ...
        if not target_url:
            logging.error(
                "Target endpoint not found in configuration. Cannot send request."
            )
            return None
        ...
        if self.server_process is None or self.server_process.poll() is not None:
            ...
            logging.error(
                f"Server not running or crashed (exit code: {exit_code}) before sending request. Input: {input_repr}. Attrib: {self.last_successful_input_repr}"
            )
            ...
            if self.last_successful_input is not None:
                ...
                bug_report = {
                    "type": "crash",
                    "input": self.last_successful_input,
                    "exit_code": exit_code,
                    "path_hash": self.last_successful_input_path_hash,
                    "trigger": "pre_request_check",
                }
                ...
                self.last_successful_input_path_hash = (
                    "N/A_POST_CRASH"  # Reset path hash attribution
                )
        ...
        try:
            response = requests.post(
                target_url, headers=headers, json=input_data, timeout=request_timeout
            )  # Use config timeout
            ...
            if elapsed_time > request_timeout * 0.95:
                logging.warning(
                    f"Request near timeout ({elapsed_time:.2f}s / {request_timeout}s). Input: {input_repr}"
                )

            # Check for HTTP errors (client/server)
            if response.status_code >= 400:
                logging.error(
                    f"HTTP {response.status_code} for {input_repr}. Response: {response.text[:500]}"
                )
            ...
            elapsed_time = time.time() - start_time
            logging.error(
                f"Request timed out after {elapsed_time:.2f}s. Input: {input_repr}"
            )
            ...
            if self.server_process and self.server_process.poll() is not None:
                ...
                logging.error(
                    f"Server process found terminated (exit code: {exit_code}) after request timeout. Input: {input_repr}"
                )
                ...
                logging.warning(
                    f"Server process still running after request timeout. Input: {input_repr}"
                )
                ...
                if psutil and self.server_process:
                    try:
                        ...
                        memory_usage_mb = mem_info.rss / (
                            1024 * 1024
                        )  # Convert RSS bytes to MB
                        # Read memory threshold from config, default to 1024 MB
                        MEMORY_THRESHOLD_MB = self.config.get(
                            "memory_threshold_mb", 1024
                        )

                        logging.debug(
                            f"Process {pid} memory usage: {memory_usage_mb:.2f} MB (Threshold: {MEMORY_THRESHOLD_MB} MB)"
                        )

                        if memory_usage_mb > MEMORY_THRESHOLD_MB:
                            timeout_type = "timeout_memory_dos"
                            logging.error(
                                f"High memory usage detected ({memory_usage_mb:.2f} MB > {MEMORY_THRESHOLD_MB} MB) after timeout. Potential DoS. Input: {input_repr}"
                            )
                        # else: timeout_type remains "timeout_alive"

                    except psutil.NoSuchProcess:
                        logging.warning(
                            f"psutil: Process {pid} not found during memory check (likely terminated between checks)."
                        )
                        # Process died between poll() and memory check, treat as crash?
                        # For simplicity, keep as timeout_alive for now, but could refine.
                    except Exception as e:
                        logging.warning(
                            f"psutil: Error checking memory for process {pid}: {e}"
                        )
                # --- End Memory Check ---
            ...
            self.last_successful_input_repr = "N/A"
            self.last_successful_input_path_hash = (
                "N/A_POST_TIMEOUT"  # Reset path hash attribution
            )
            return None
        except requests.exceptions.ConnectionError as e:
            logging.error(
                f"Connection error: {e}. Input: {input_repr}. Server might have crashed."
            )
            ...
            if self.server_process and self.server_process.poll() is not None:
                ...
                logging.error(
                    f"Server confirmed crashed after ConnectionError (exit code: {exit_code}). Input: {input_repr}"
                )
            ...
            self.last_successful_input_repr = "N/A"
            self.last_successful_input_path_hash = (
                "N/A_POST_CONN_ERROR"  # Reset path hash attribution
            )
            return None
            ...
            logging.error(
                f"Unexpected error during send_request: {e}. Input: {input_repr}",
                exc_info=True,
            )
            bug_report = {
                "type": "unexpected_request_error",
                "input": input_data,
                "error": str(e),
                "path_hash": self.last_successful_input_path_hash,
            }
    ...
    def hash_arcs_with_counts(
        self, arc_hit_counts: Dict[Tuple[str, int, int], int]
    ) -> str:
    ...
    def get_arc_hit_counts(
        self, cov_data: Optional[CoverageData]
        ...
        try:
            ...
            for filename in measured_files:
                ...
                try:
                    ...
                    logging.error(
                        f"Failed to get arcs for file {filename}: {arc_err}",
                        exc_info=True,
                    )
    ...
    def run(self) -> None:
        ...
        try:  # Wrap in try/finally to ensure server stops
            ...
            logging.info(
                f"Fuzzer started (Sequential Mode). Max iterations: {self.max_iterations}, Max mutations: {self.total_mutation_limit}"
            )
                ...
                json.dumps(
                    {
                        ...
                        "total_mutation_limit": self.total_mutation_limit,
                    }
                )
            )
            exp_logger.info(
                json.dumps(
                    {
                        ...
                        "columns": [
                            ...
                            "execution_success",
                            "mutation_path_hash",
                            "mutation_unique_arcs_count",
                            "new_unique_arcs_count",
                            "is_mutation_interesting",
                            "added_to_queue",
                            "total_unique_arcs",
                            "total_unique_paths",
                        ],
            ...
            while (
                ...
                and self.mutation_count_total < self.total_mutation_limit
                and (time.time() - self.start_time) < self.run_duration_seconds
            ):

                # Check duration limit again inside loop
                if (time.time() - self.start_time) >= self.run_duration_seconds:
                    logging.info(
                        f"Run duration limit ({self.run_duration_hours} hours) reached. Stopping."
                    )
                    break
                ...
                seed_s_pre_select = (
                    chosen_seed_dict.get("s", 1) - 1
                )  # s was just incremented
                seed_f_pre_mutate = chosen_seed_dict.get(
                    "f", 0
                )  # f is incremented below

                # Calculate energy for this seed
                energy = 1  # Default energy
                current_avg_f = 0.0  # Default if not calculated
                fixed_energy_value = (
                    100  # Define a fixed energy value for non-prioritized runs
                )
                ...
                if self.use_seed_prioritization:
                    ...
                    energy = self.power_schedule.assignEnergy(
                        chosen_seed_dict, current_avg_f
                    )
                    logging.debug(
                        f"Seed prioritization enabled. Avg f={current_avg_f:.2f}. Calculated energy: {energy}"
                    )
                else:
                    # Assign a fixed, moderate energy if prioritization is disabled
                    energy = fixed_energy_value
                    logging.debug(
                        f"Seed prioritization disabled, using fixed energy: {fixed_energy_value}"
                    )
                ...
                logging.info(
                    f"Selected seed (s={seed_s_pre_select}, f={seed_f_pre_mutate}): {seed_data_repr}"
                )
                ...
                if energy == 0:
                    ...
                    log_entry = {
                        ...
                        "seed_f_pre": seed_f_pre_mutate,
                        "mutated_input": "N/A (Skipped)",
                ...
                mutations_to_attempt = min(
                    energy, self.total_mutation_limit - self.mutation_count_total
                )
                logging.info(
                    f"Attempting {mutations_to_attempt} mutations for this seed."
                )

                for mutation_index_in_iteration in range(mutations_to_attempt):
                    if self.mutation_count_total >= self.total_mutation_limit:
                        logging.warning(
                            f"Reached total mutation limit ({self.total_mutation_limit}). Stopping early."
                        )
                    ...
                    logging.debug(
                        f"Iter {iteration_count}, Mut #{self.mutation_count_total}: Input {mutated_input_repr}"
                    )
                    ...
                    if (
                        ...
                        or self.server_process.poll() is not None
                    ):
                        exit_code = (
                            self.server_process.poll() if self.server_process else "N/A"
                        )
                        logging.error(
                            f"Server crashed (exit code: {exit_code}) *before* mutation {self.mutation_count_total}. Input: {mutated_input_repr}. Attrib Input: {self.last_successful_input_repr}, Attrib Path: {self.last_successful_input_path_hash}"
                        )
                        if self.last_successful_input is not None:
                            # Attribute crash to last successful input and its path hash
                            bug_report = {
                                ...
                                "exit_code": exit_code,
                                "path_hash": self.last_successful_input_path_hash,
                                "trigger": "pre_mutation_check",
                            }
                            # Log using the helper method
                            self._log_unique_error(
                                bug_report, self.last_successful_input_path_hash
                            )
                            # if bug_report not in self.bugs: self.bugs.append(bug_report) # Optional
                        ...
                        if (
                            ...
                            logging.critical(
                                f"Failed to restart server after crash detected before mutation {self.mutation_count_total}. Stopping."
                            )
                            ...
                            log_entry = {
                                ...
                                "execution_success": False,
                                "mutation_path_hash": "CRASH_RESTART_FAIL",
                                "mutation_unique_arcs_count": 0,
                                "new_unique_arcs_count": 0,
                                "is_mutation_interesting": False,
                                "added_to_queue": False,
                                "total_unique_arcs": len(self.global_unique_arcs),
                                "total_unique_paths": len(
                                    self.global_unique_path_hashes
                                ),
                            }
                            exp_logger.info(json.dumps(log_entry, ensure_ascii=False))
                    ...
                    if self.server_process and self.server_process.poll() is None:
                        ...
                        try:
                            ...
                            logging.error(
                                f"Unexpected exception during send_request call for mutation {self.mutation_count_total}: {e}\nInput: {mutated_input_repr}",
                                exc_info=True,
                            )
                            # Use last known good path hash for this unexpected error
                            bug_report = {
                                "type": "send_request_call_exception",
                                "input": mutated_input,
                                "error": str(e),
                                "path_hash": self.last_successful_input_path_hash,
                            }
                            ...
                            try:
                                ...
                                logging.error(
                                    f"Coverage stop/combine error after mutation {self.mutation_count_total}: {ce}"
                                )
                            except Exception as e:
                                logging.error(
                                    f"Unexpected error during coverage stop/combine: {e}",
                                    exc_info=True,
                                )
                        ...
                        if (
                            ...
                            exit_code = self.server_process.poll()
                            logging.error(
                                f"Server crashed (exit code: {exit_code}) *after* mutation {self.mutation_count_total}. Input: {mutated_input_repr}."
                            )
                            ...
                            execution_success = (
                                False  # Don't process coverage if server crashed
                            )
                            self.last_successful_input = None  # Reset attribution
                            self.last_successful_input_repr = "N/A"
                            self.last_successful_input_path_hash = (
                            ...
                            if (
                                ...
                                logging.critical(
                                    f"Failed to restart server after crash during mutation {self.mutation_count_total}. Stopping."
                                )
                                # Log failure to experiment log before breaking
                                log_entry = {
                                    "event": "mutation_log",
                    ...
                    if not server_crashed_this_run:
                        try:
                            ...
                            if cov_data and cov_data.measured_files():
                                ...
                                if arc_hit_counts:
                                    mutation_path_hash = self.hash_arcs_with_counts(
                                        arc_hit_counts
                                    )  # Calculate hash
                                    mutation_arcs_set = set(
                                        arc_hit_counts.keys()
                                    )  # Calculate arcs
                                    mutation_unique_arcs_count = len(mutation_arcs_set)
                                    new_arcs = (
                                        mutation_arcs_set - self.global_unique_arcs
                                    )
                                    ...
                                    is_new_path = (
                                        mutation_path_hash
                                        not in self.global_unique_path_hashes
                                    )
                                    if is_new_path:
                                        self.global_unique_path_hashes.add(
                                            mutation_path_hash
                                        )
                                    if new_unique_arcs_count > 0:
                                        self.global_unique_arcs.update(
                                            mutation_arcs_set
                                        )
                                    ...
                                    is_mutation_interesting = self.is_interesting(
                                        arc_hit_counts, self.global_coverage_map
                                    )
                                    ...
                                    if is_new_path or is_mutation_interesting:
                                        reason = []
                                        if is_new_path:
                                            reason.append("new_path")
                                        if is_mutation_interesting:
                                            reason.append("new_bucket")
                                        interesting_log_entry = {
                                            ...
                                            "total_unique_arcs": len(
                                                self.global_unique_arcs
                                            ),
                                            "total_unique_paths": len(
                                                self.global_unique_path_hashes
                                            ),
                                        }
                                        interesting_logger.info(
                                            json.dumps(
                                                interesting_log_entry,
                                                ensure_ascii=False,
                                            )
                                        ...
                                        if self.use_coverage_feedback:
                                            logging.info(
                                                f"Mutation {self.mutation_count_total} interesting and feedback enabled. Adding to queue."
                                            )
                                            self.seed.queue.append(
                                                {
                                                    "data": mutated_input.copy(),
                                                    "s": 0,
                                                    "f": 0,
                                                }
                                            )
                                            ...
                                            logging.debug(
                                                f"Mutation {self.mutation_count_total} interesting but feedback disabled. Not adding to queue."
                                            )
                                else:  # No arcs found
                                    ...
                                    logging.warning(
                                        f"No arc hit counts extracted for mutation {self.mutation_count_total}: {mutated_input_repr}"
                                    )
                                ...
                                logging.warning(
                                    f"No coverage data measured or loaded for mutation {self.mutation_count_total}: {mutated_input_repr}"
                                )
                        except CoverageException as ce:
                            logging.error(
                                f"Failed to get/load coverage data after mutation {self.mutation_count_total}: {ce}"
                            )
                            mutation_path_hash = "COVERAGE_ERROR"
                        except Exception as e:
                            logging.error(
                                f"Unexpected error processing coverage for mutation {self.mutation_count_total}: {e}",
                                exc_info=True,
                            )
                        ...
                        new_unique_arcs_count = 0
                        is_mutation_interesting = (
                            False  # Ensure these are reset if server crashed
                        )
                        added_to_queue = False
                    ...
                    if server_crashed_this_run:
                        ...
                        bug_report = {
                            ...
                            "input": mutated_input,
                            "exit_code": exit_code,
                            "trigger": "post_mutation",
                            "path_hash": mutation_path_hash,
                        }
                        # Log using the helper method
                    ...
                    log_entry = {
                        ...
                        and not server_crashed_this_run,  # Mark false if crashed
                        "mutation_path_hash": (
                            mutation_path_hash
                            if not server_crashed_this_run
                            else "CRASHED"
                        ...
                        "new_unique_arcs_count": (
                            new_unique_arcs_count if not server_crashed_this_run else 0
                        ),
                        "is_mutation_interesting": (
                            is_mutation_interesting
                            if not server_crashed_this_run
                    ...
                    if server_crashed_this_run:
                        logging.warning(
                            f"Server crashed during mutation {self.mutation_count_total}, continuing to next mutation after restart."
                        )
                ...
                logging.info(
                    f"Iteration {iteration_count} finished processing seed. Total Mutations: {self.mutation_count_total}. Queue Size: {len(self.seed.queue)}"
                )
            ...
            logging.error(
                f"Unhandled exception in main fuzzing loop: {e}", exc_info=True
            )
            # Use "FUZZER_LOOP" as path hash for fuzzer-level exceptions
            bug_report = {
                "type": "fuzzer_exception",
                "error": str(e),
                "traceback": traceback.format_exc(),
                "path_hash": "FUZZER_LOOP",
            }
            ...
            exp_logger.error(
                json.dumps(
                    {"event": "fuzzer_exception", "error": str(e)}, ensure_ascii=False
                )
            )
        finally:
            ...
            unique_errors_filename = self.config.get(
                "unique_errors_file", "unique_errors.jsonl"
            )
            unique_errors_filepath = os.path.join(
                self.get_run_output_dir(), unique_errors_filename
            )
            logging.info(
                f"Saving {len(self.unique_error_details)} unique errors found to {unique_errors_filepath}..."
            )
            try:
                with open(
                    unique_errors_filepath, "w"
                ) as f:  # Overwrite file at the end
                    for error_detail in self.unique_error_details:
                        try:
                            ...
                            logging.error(
                                f"Could not serialize unique error detail: {json_err}. Detail: {error_detail}"
                            )
                            f.write(
                                json.dumps(
                                    {
                                        "error": "Serialization failed",
                                        "original_detail_repr": repr(error_detail),
                                    }
                                )
                                + "\n"
                            )
                logging.info(
                    f"Successfully saved unique errors to {unique_errors_filepath}"
                )
            except Exception as e:
                logging.error(
                    f"Failed to save unique errors to {unique_errors_filepath}: {e}",
                    exc_info=True,
                )
            ...
            try:
                ...
                self.cov.combine(
                    strict=False
                )  # Use strict=False to ignore errors if no data file found

                # Load the combined data from the file system
                final_cov_data = CoverageData(
                    basename=self.coverage_data_file_path
                ...
                if final_cov_data and final_cov_data.measured_files():
                    ...
                    final_coverage_arcs = len(
                        final_arc_hit_counts
                    )  # Count unique arcs found in the whole run
                    logging.info(
                        f"Final coverage processing: Found {final_coverage_arcs} unique arcs."
                    )
                ...
                logging.error(
                    f"Unexpected error processing final coverage: {e}", exc_info=True
                )
            ...
            final_summary = {
                ...
                "final_total_unique_paths": len(
                    self.global_unique_path_hashes
                ),  # Path hashes are not reliable now
                "final_seed_queue_size": len(self.seed.queue) if self.seed else 0,
                "final_bug_count": len(
                    self.unique_error_details
                ),  # Use unique errors count
            }
            exp_logger.info(
                json.dumps(final_summary, ensure_ascii=False)
            )  # Log summary to exp log
            ...
            logging.info(
                f"Final Unique Arcs Found (End of Run): {final_coverage_arcs}"
            )  # Report final arcs
            logging.info(
                f"Final Unique Path Hashes Found (Not Reliable): {len(self.global_unique_path_hashes)}"
            )
            logging.info(
                f"Final Seed Queue Size: {len(self.seed.queue) if self.seed else 0}"
            )  # Check if seed exists
            logging.info(
                f"Total Unique Errors Found: {len(self.unique_error_details)}"
            )  # Log count of unique errors
            ...
            if self.unique_error_details:
                ...
                exp_logger.info(
                    json.dumps(
                        {
                            "event": "final_unique_errors_summary",
                            "count": len(self.unique_error_details),
                        }
                    )
                )
                ...
                if len(self.unique_error_details) > 5:
                    logging.info(
                        f"... and {len(self.unique_error_details) - 5} more unique errors (see {unique_errors_filepath})."
                    )
            else:
                logging.info("No unique errors recorded.")
                exp_logger.info(
                    json.dumps({"event": "final_unique_errors_summary", "count": 0})
                )
            ...
            for line_num, line in enumerate(f):
                ...
                try:
                    ...
                    if isinstance(seed_data, dict):
                        ...
                        logging.warning(
                            f"Skipping non-dictionary item in seed file {file_path} at line {line_num + 1}: {line}"
                        )
                except json.JSONDecodeError as e:
                    logging.error(
                        f"Error decoding JSON in seed file {file_path} at line {line_num + 1}: {e}. Line: '{line}'"
                    )
    except Exception as e:
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="DjangoWebApplication/fuzzer/fuzzer_config.yaml",
